...
#include "action_tp_mult.H"


// Cast data read from AXI input port to transprecision values
static void mbus_to_tp(snap_membus_512_t *data_read, mat_elmt_t *table_tp_in)
{
	union {
		uint64_t     value_u;
		mat_elmt_t   value_d;
...
#pragma HLS PIPELINE
	   loop_m2d2: for(int j = 0; j < MAX_NB_OF_TP_PERDW; j++)
	   {
		value_u = (uint64_t)data_read[i]((8*sizeof(mat_elmt_t)*(j+1))-1, (8*sizeof(mat_elmt_t)*j));
		table_tp_in[i*MAX_NB_OF_TP_PERDW + j] = value_d;
	   }

}

// Cast transprecision values to AXI output port format (64 Bytes)
static void  tp_to_mbus(mat_elmt_t *table_tp_out, snap_membus_512_t *data_to_be_written)
{
	union {
		mat_elmt_t   value_d;
		uint64_t     value_u;
...
#pragma HLS PIPELINE
	   loop_d2m2: for(int j = 0; j < MAX_NB_OF_TP_PERDW; j++)
	   {
		value_d = table_tp_out[i*MAX_NB_OF_TP_PERDW + j];
			  ...
			  action_reg *act_reg)
{
	...
	mat_elmt_t data_in[MAX_NB_OF_TP_READ], data_out[MAX_NB_OF_TP_READ];
	...
	// READ MAX_NB_OF_TP_READ of 8 or 4  bytes on port din_gmem at address i_idx and put result in buffer_in
	memcpy(buffer_in, (snap_membus_512_t*)(din_gmem + i_idx), MAX_NB_OF_TP_READ*sizeof(mat_elmt_t));
...
#pragma HLS PIPELINE
	  ...
	  switch (tp_mode) {
	    ...
	    case AP_UINT:
		static tp_elmt_t_uint a_uint, b_uint, c_uint, d_uint;
	        a_uint = (tp_elmt_t_uint)data_in[3*i];
		b_uint = (tp_elmt_t_uint)data_in[(3*i)+1];
		c_uint = (tp_elmt_t_uint)data_in[(3*i)+2];
		d_uint = a_uint * b_uint * c_uint;
		data_out[i] =  (mat_elmt_t)d_uint;
	    break;
	    case AP_INT:
		static tp_elmt_t_int a_int, b_int, c_int, d_int;
	        a_int = (tp_elmt_t_int)data_in[3*i];
		b_int = (tp_elmt_t_int)data_in[(3*i)+1];
		c_int = (tp_elmt_t_int)data_in[(3*i)+2];
		d_int = a_int * b_int * c_int;
		data_out[i] =  (mat_elmt_t)d_int;
	    break;	  
	    case AP_UFIXED:
		static tp_elmt_t_ufixed a_ufixed, b_ufixed, c_ufixed, d_ufixed;
	        a_ufixed = (tp_elmt_t_ufixed)data_in[3*i];
		b_ufixed = (tp_elmt_t_ufixed)data_in[(3*i)+1];
		c_ufixed = (tp_elmt_t_ufixed)data_in[(3*i)+2];
		d_ufixed = a_ufixed * b_ufixed * c_ufixed;
		data_out[i] =  (mat_elmt_t)d_ufixed;
	    break;
	    case AP_FIXED:
		static tp_elmt_t_fixed a_fixed, b_fixed, c_fixed, d_fixed;
	        a_fixed = (tp_elmt_t_fixed)data_in[3*i];
		b_fixed = (tp_elmt_t_fixed)data_in[(3*i)+1];
		c_fixed = (tp_elmt_t_fixed)data_in[(3*i)+2];
		...
		c_half = (tp_elmt_t_float)data_in[(3*i)+2];
		...
		c_float = (tp_elmt_t_float)data_in[(3*i)+2];
		...
		c_double = (tp_elmt_t_float)data_in[(3*i)+2];
		d_double = a_float * b_float * c_double;
		data_out[i] =  (mat_elmt_t)d_double;
	    break;    
	    default:
		data_out[i] = 0;
	  }  
	...
	// CONVERT the doubles to a format that can be sent to host memory
	 tp_to_mbus(data_out, buffer_out);
	...
	//memcpy(dout_gmem + o_idx, &buffer_out, MAX_NB_OF_TP_READ*sizeof(mat_elmt_t));
