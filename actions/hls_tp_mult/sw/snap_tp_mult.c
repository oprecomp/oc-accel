...
static void usage(const char *prog)
{
	printf("Usage: %s [-h] [-v, --verbose] [-V, --version]\n"
	       ...
	       "  -p, --precision           transprecision mode.\n"       
	       ...
	       "  snap_tp_mult. ...\n"
	       "\n",
	       prog, MAX_NB_OF_TP_READ);
}

static void snap_prepare_tp_mult(struct snap_job *cjob,
				 struct tp_mult_job *mjob,
				 void *addr_in,
				 uint32_t size_in,
				 uint8_t type_in,
				 void *addr_out,
...
{
	fprintf(stderr, "  prepare tp_mult job of %ld bytes size\n", sizeof(*mjob));
	...
	mjob->transprecision_mode = transprecision_mode;
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		case 'n':
			...
			break;
		case 'p':
			transprecision_mode = strtol(optarg, (char **)NULL, 0);
			// check max: bounded to MAX_NB_OF_TP_READ in this example
	...
	printf("PARAMETERS:\n"
	       ...
	       inputs_to_process, transprecision_mode);
	...
	snap_prepare_tp_mult(&cjob, &mjob,
	...
	fprintf(stdout, "Action processing tp_mult took %lld usec\n",
	...
	fprintf(stdout, "In this example %d transprecision numbers (%d bytes large) are used as inputs " \
	...
	fprintf(stdout, "Max value of inputs is set as %d (set in header file of include directory)\n", 
		MAX_NB_OF_TP_READ);
	fprintf(stdout, "Operation is the multiplication of 3 transprecision numbers \n");
