...
function test_tp_mult {
    local size=$1

    if [ ! -f "${ACTION_ROOT}/sw/snap_tp_mult" ]; then
       echo "ERROR: please compile 'snap_tp_mult' before execution (or run 'make apps')"
       exit 1
    fi

    echo -n "Doing snap_tp_mult "
    cmd="snap_tp_mult -C${snap_card} -n 16 -w >> snap_tp_mult.log 2>&1"
    eval ${cmd}
    if [ $? -ne 0 ]; then
	cat snap_tp_mult.log
	echo "cmd: ${cmd}"
	echo "failed"
	exit 1
    fi
...
rm -f snap_tp_mult.log
touch snap_tp_mult.log

if [ "$duration" = "NORMAL" ]; then
  test_tp_mult 
  fi

rm -f *.bin *.bin *.out snap_tp_mult.log
echo "Test OK"
exit 0
